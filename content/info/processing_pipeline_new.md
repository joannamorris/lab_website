...
[`ERP_pipeline.m`](/files/GenericScripts/ERP_pipeline.m)
...
Use [`HC_create_chan_locs.m`](/files/GenericScripts/HC_create_chan_locs.m) to create:
...
Use [`PC_create_chan_locs.m`](/files/GenericScripts/PC_create_chan_locs.m) to create:
...
[`ERP_pipeline.m`](/files/GenericScripts/ERP_pipeline.m)
 should select the channel-location and rereference files according to the recording site.
...
[`BrainVision_load_vhdr.m`](/files/GenericScripts/BrainVision_load_vhdr.m)
...
## 8. Recommended preprocessing pipeline and Runtime parameters

Script:`ERP_pipeline.m`



When [`ERP_pipeline.m`](/files/GenericScripts/ERP_pipeline.m) starts, enter the parameters for the current study/task.
...
Because `eeg_interp` may append a restored channel rather than return it to its original array position, [`ERP_pipeline.m`](/files/GenericScripts/ERP_pipeline.m) immediately restores the original channel order by label and reinstates the saved `chanlocs` structure. No separate channel-reordering step is required for newly processed data.
...
## 10. Pipeline outputs and QC files
...
## 11. Artifact-rejection QC (`VSL2`PROJECT SPECIFIC)
...
## 12. Review individual subjects before continuing
...
## 13. Channel-order restoration

Channel order is now restored **inside [`ERP_pipeline.m`](/files/GenericScripts/ERP_pipeline.m) immediately after interpolation**.
...
## 14. Create derived bins (`VSL2`PROJECT SPECIFIC)
...
Bin operations do not require a separate channel-reordering step when the input ERP was produced by the current [`ERP_pipeline.m`](/files/GenericScripts/ERP_pipeline.m). Channel order may still be inspected manually as a QC check.

---

## 15. Finalize subject lists
...
[`ERP_grand_average.m``](/files/GenericScripts/ERP_grand_average.m)
...
## 17 Export mean ERP amplitudes

### Script

[`ERP_measure_amp.m`](/files/GenericScripts/ERP_measure_amp.m)
...
## 21. Items to verify before a new PC batch
...
2. **Montage selection**
   ...
   Confirm that [`ERP_pipeline.m`](/files/GenericScripts/ERP_pipeline.m) is run with `location = 'pc'`. The pipeline then selects `M21_PC_31ch.ced`, uses `reref_eq_pchpl.txt`, expects 31 input channels, and uses channels `1:31` for epoch-level artifact rejection.
...
A separate channel-reorder script is not required for newly processed PC data because channel order is restored inside [`ERP_pipeline.m`](/files/GenericScripts/ERP_pipeline.m).
